import { PullDemuxerBase, AUDIO_STREAM_TYPE, VIDEO_STREAM_TYPE } from '/js/player/lib/pull_demuxer_base.js'
...
class MP4Source {
  ...
  getAudioSpecificConfig() {
    // TODO: make sure this is coming from the right track. 
    let audioTrack = null
    for (let track = 0; track < this.file.moov.traks.length; track++) {
      if (!this.file.moov.traks[track].mdia.minf.stbl.stsd.entries[0].esds) { continue }

      audioTrack = this.file.moov.traks[track];
      break
    }

    if (audioTrack == null) {
      console.log("no audio track in file")
    }

    let audioDesc = audioTrack.mdia.minf.stbl.stsd.entries[0].esds.esd.descs[0]
    // 0x04 is the DecoderConfigDescrTag. Assuming MP4Box always puts this at position 0.
    console.assert(audioDesc.tag == 0x04);
    // 0x40 is the Audio OTI, per table 5 of ISO 14496-1
    console.assert(audioDesc.oti == 0x40);
    // 0x05 is the DecSpecificInfoTag
    console.assert(audioDesc.descs[0].tag == 0x05);

    return audioDesc.descs[0].data;
  }
